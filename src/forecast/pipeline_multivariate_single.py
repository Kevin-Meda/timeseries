"""Multivariate forecasting pipeline using feature store features only."""
...
def run_multivariate_single_pipeline(
    ...
    """Run the multivariate single-product features pipeline.

    Uses Prophet, XGBoost, and Chronos with features from feature store.
    Outputs to: output/multivariate_single/{project}/
    ...
    logger, pm, timestamp = setup_pipeline_run(
        ...
        pipeline_type="multivariate_single",
    )
    ...
    # Load feature store
    feature_store, features_by_product = load_feature_store(config, cleaned_data)
    if not features_by_product:
        logger.error(
            "No features loaded from feature store. "
            "Check that feature_store_path is correct and contains features."
        )
        return {"error": "No features loaded from feature store"}

    logger.info(f"Loaded features for {len(features_by_product)} products")
    ...
    logger.info("Multivariate single-product pipeline completed successfully")
...
def main() -> int:
    """CLI entry point for multivariate single-product pipeline."""
    parser = argparse.ArgumentParser(
        description="Multivariate Single-Product Pipeline - Prophet, XGBoost, Chronos with feature store",
    )
    ...
    try:
        results = run_multivariate_single_pipeline(
        ...
        print(f"\nMultivariate single-product pipeline completed successfully!")
